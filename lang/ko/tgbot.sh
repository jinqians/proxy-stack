# ko/tgbot.sh — Telegram Bot module Korean messages (assign MSG[...] only; do not declare).
...
MSG[tgbot.token.gen_failed]="바인딩 토큰 생성에 실패해 아무것도 저장하지 않았습니다. 빈 토큰은 해당 포트의 검증을 건너뛰게 만들므로 실패하는 편이 안전합니다."
