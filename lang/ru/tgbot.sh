...
MSG[tgbot.token.gen_failed]="Не удалось создать токен привязки, ничего не записано. Пустой токен отключил бы проверку для этого порта, поэтому безопаснее завершиться ошибкой."
