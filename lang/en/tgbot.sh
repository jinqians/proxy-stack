# en/tgbot.sh — Telegram Bot module English messages (assign MSG[...] only; do not declare).
...
MSG[tgbot.token.gen_failed]="Failed to generate a bind token; nothing was written. An empty token would make the bind check skip verification for that port, so failing is the safer outcome."
