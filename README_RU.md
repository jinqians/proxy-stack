...
**Универсальный менеджер прокси-сервера для Linux · два ядра: Xray / sing-box**
...
**Proxy Stack Manager (PSM)** — универсальный инструмент управления прокси-сервером Linux на чистом Bash. Одной командой `psm` устанавливаются VLESS Reality / Vision / XHTTP, Shadowsocks, Hysteria2, Snell, AnyTLS и другие протоколы; встроены **два ядра — Xray и sing-box**; централизованно управляются Nginx, SSL-сертификаты, ретрансляция realm, мониторинг трафика по узлам с уведомлениями Telegram-бота, защита VPS, приложения Docker и сервисы Cloudflare.
...
- **Два ядра на выбор**: Xray и sing-box управляются параллельно; входящие протоколы, маршрутизация и исходящие настраиваются независимо для каждого ядра
...
══════════════════════════════════════════════════════════════
                  JQ's Proxy Stack Manager
══════════════════════════════════════════════════════════════
   1. Управление системой            11. Ретрансляция (realm)
   2. Управление sing-box            12. Cloudflare DDNS
   3. Управление Xray                13. Управление Docker
   4. Управление Snell               14. Управление трафиком
   5. Управление ss-rust             15. Telegram-бот
   6. Управление Hysteria2           16. Управление резервными копиями
   7. Управление Nginx               17. Восстановить резервную копию
   8. Управление сайтами             18. Обновить PSM
   9. Управление SSL-сертификатами   19. Усиление безопасности
  10. Показать все узлы              20. Язык / Language
──────────────────────────────────────────────────────────────
   0. Выход
══════════════════════════════════════════════════════════════
```
...
- **Управление системой** — управление перегрузкой BBR, тюнинг сети sysctl, файрвол, DNS, часовой пояс
...
Выберите «20. Язык / Language» в главном меню — переключение между 简体中文 / English / 한국어 / Русский сохраняется. Команда `PSM_LANG=ru psm` временно переопределяет язык только для текущей сессии.
...
Это независимые параллельные ядра. Сторона Xray богаче инструментами (watchdog живости, поиск доменов-приманок, учёт трафика); сторона sing-box покрывает больше протоколов (AnyTLS, нативный входящий Snell) и имеет более современную конфигурацию маршрутизации. Используйте одно из них или оба сразу — каждое управляет своими портами и узлами.
