...
flowchart TD
    ...
    B -->|"SNI = e.example.com (ваш домен)"| F["HTTPS-сайт-приманка<br/>127.0.0.1:8443"]
    B -->|"SNI не совпал ни с одной записью"| X["Соединение разрывается<br/>апстрим не открывается вовсе"]
...
- **Неизвестный SNI отбрасывается**: таблица маршрутизации знает только явно смонтированные домены, поэтому сканер с произвольным SNI получает разрыв соединения, а к апстриму не уходит ни одного подключения — эта «чёрная дыра» заодно закрывает путь к использованию вашего сервера как бесплатного ретранслятора (см. вопрос о целях маскировки за CDN)
...
manager.sh --vpngate-watchdog      # Однократная проверка домашнего туннеля VPNGate со сменой узла при обрыве
manager.sh --ruleset-update        # Однократное обновление наборов правил (Xray перезапускается при изменении)
...
- **Умный поиск доменов-приманок Reality** — при настройке SNI маскировки Reality / XHTTP поисковые движки киберпространства (Netlas / Quake / ZoomEye / FOFA, ваш собственный API-ключ, хватает бесплатных лимитов) автоматически находят реальные сайты TLS 1.3 в **том же ASN / том же дата-центре**, что и ваш сервер: близкие, малоизвестные, без заезженных доменов крупных брендов. Хост в вашем же ASN к тому же не является границей CDN, что снимает риск злоупотребления, описанный в частых вопросах. Кандидаты принимаются только после локальной проверки реальным рукопожатием (TLS 1.3 / X25519 / совпадение сертификата) и могут пакетно добавляться в пул живости выше. **Локальное сканирование портов не выполняется вовсе** (чтобы не провоцировать abuse-жалобы хостера) — поиск идёт по датасетам движков; без настроенного движка происходит возврат к ручному вводу
...
- **Домашний (residential) выход VPNGate** — отбирает настоящие домашние широкополосные IP из публичного списка VPNGate (пакетная классификация через ip-api отсеивает ЦОД и собственные релеи VPNGate), поднимает отдельный туннель openvpn и подключает его как исходящее с fwmark: сервисы, которые судят по принадлежности IP (Netflix / ChatGPT ...), видят домашний выход, а собственный маршрут по умолчанию и SSH сервера не затрагиваются (туннель пишет только в отдельную таблицу маршрутизации). При обрыве туннеля попавший под правила трафик просто не проходит, а не утекает через IP из ЦОД; переключение происходит автоматически **внутри выбранной вами страны** (страна выбирается из списка тех, где действительно есть узлы), поэтому страна выхода не меняется незаметно. Xray / sing-box / mihomo используют один туннель, поэтому смена домашнего IP не меняет конфигурацию ни одного ядра
- **Наборы правил по подписке** — вставьте URL списка правил сообщества (OpenAI.list и подобные), выберите выход — и описанный там трафик пойдёт через него. У Xray нет механизма наборов правил, поэтому правила встраиваются прямо в `config.json` (домены и IP — двумя отдельными правилами: поля внутри одного правила объединяются по AND, и вместе они не сработали бы никогда), из-за чего это единственное ядро, требующее перезапуска; ежедневное обновление пересобирает и перезапускает его только при реальном изменении содержимого
...
- **Стабильный и предварительный каналы для Xray** — XTLS помечает все релизы после v26.3.27 как prerelease, поэтому стабильный канал может отставать на месяцы, а предварительный берёт свежую сборку. При выборе предварительного канала выводится предупреждение: начиная с v26.4.13 REALITY отклоняет клиентов с ядром старее v26.3.27 (включая ядра во многих мобильных приложениях); ослабить это можно для каждого узла в меню Reality
- **Стабильный и предварительный каналы** — канал ядра выбирается при установке и обновлении. По умолчанию стабильный; предварительный ставит последний beta/rc и сейчас является единственным способом получить протоколы, ещё не стабилизированные апстримом (входящий Snell требует 1.14+, а 1.14 пока в beta). Возврат на стабильный канал при наличии узлов Snell блокируется с пояснением, чтобы конфигурация не осталась невалидной с остановленной службой
...
- **Наборы правил по подписке** — вставьте URL списка правил сообщества (OpenAI.list и подобные), выберите выход — и описанный там трафик пойдёт через него. Используется родной `rule_set` (локальный source-файл, ядро перечитывает его само начиная с 1.10), поэтому обновление ничего не перезапускает. Перед применением показывается отчёт: сколько правил пригодно и какие клиентские типы (PROCESS-NAME и т. п.) отброшены; при обновлении число правил сравнивается, и резкие скачки отклоняются до проверки
- **Домашний выход VPNGate** — тот же туннель, что и у Xray; исходящее — `direct` + `routing_mark`, смена узла не требует правки конфигурации (в sing-box 1.12+ используется `domain_resolver` для фиксации IPv4)
...
- **Наборы правил по подписке** — на стороне mihomo создаётся родная запись `rule-providers`, которую ядро обновляет по своему interval; типы вроде `IP-ASN`, понятные только mihomo, при этом сохраняются
- **Домашний выход VPNGate** — тот же туннель, что и у Xray; создаётся прокси `type: direct` с `routing-mark` и `ip-version: ipv4`, смена узла не требует правки конфигурации
...
│   ├── vpngate/          # Домашний выход VPNGate (список, классификация, туннель openvpn, подключение ядер)
│   ├── ruleset/          # Наборы правил по подписке (загрузка, разбор, проверка, применение)
...
### В логе появляется предупреждение «REALITY: Listening on non-443 ports» — это проблема?

В режиме мультиплексирования порта 443 — нет, это ожидаемо. Начиная с v26.3.27 Xray предупреждает о любом входящем REALITY, слушающем порт, отличный от 443, потому что при прямом подключении такой порт действительно легче распознаётся GFW. Но в режиме мультиплексирования PSM узел слушает петлевой порт на `127.0.0.1` (1443, 2443 и т. д.), а Nginx перенаправляет на него с публичного порта 443 по SNI — **наружу открыт именно порт 443**, поэтому описанный в предупреждении риск неприменим.

Если узел не находится за разветвлением Nginx на 443, а напрямую слушает публичный порт, отличный от 443, предупреждение справедливо: перенесите его на 443 или подключите к мультиплексированию порта.

Та же версия может выдать ещё два предупреждения REALITY: когда маскировочная цель содержит `apple` / `icloud` / `microsoft` либо оканчивается на `.cn` / `.ru` / `.ir` (смените домен), и начиная с v26.4.13 — о минимальной версии ядра клиента по умолчанию (см. раздел о ядре Xray выше).

### Можно ли указать целью маскировки (`dest` / SNI) сайт за Cloudflare?

Не рекомендуется — PSM предупреждает об этом при настройке.

Маскировка REALITY работает так: любое соединение, **не прошедшее аутентификацию**, сервер пересылает цели маскировки (`dest`) как есть, чтобы зондирующий увидел полный TLS-ответ настоящего сайта. Эта пересылка безусловна — она не смотрит ни на источник, ни на то, что запрошено в ClientHello.

Проблема начинается, когда `dest` оказывается на мультиарендной границе CDN (Cloudflare, Fastly, Akamai и т. д.). Такие IP обслуживают **любого арендатора** по SNI, поэтому любому, кто просканирует ваш 443, достаточно указать в ClientHello другое имя хоста на том же CDN — и он получит туннель во весь этот CDN через вашу машину: **ваш сервер становится бесплатным пробросом портов для CDN**, за ваш трафик и ваш счёт. Документация Xray предупреждает ровно об этом.

PSM отвечает на это тремя слоями:

1. **Никакого домена-приманки по умолчанию** — во всех трёх ядрах SNI маскировки по умолчанию пуст, так что множество установок не делит один заводской дефолт (который к тому же может позже уехать за CDN). Поиск в Xray предпочитает цели в **том же ASN**, а хост в вашем же дата-центре не является границей CDN; у sing-box и mihomo поиска нет, поэтому они требуют явную цель, а неинтерактивный CLI завершается с понятным сообщением, а не строит узел на заимствованном дефолте
2. **Проба на этапе настройки** — после ввода `dest` PSM подключается к тому же IP, подставляя в SNI набор посторонних имён хостов. Если возвращается валидный сертификат на имя, которым цель не владеет, она помечается как общая граница. Проверяется именно то свойство, которое важно (обслуживает ли этот IP произвольных арендаторов?), а не диапазоны IP или списки ASN. Это **предупреждение, а не запрет** — вердикт может ошибаться, и риск оценивать вам
3. **Два подстраховочных механизма** — в режиме переиспользования порта 443 неизвестный SNI отбрасывается ещё на уровне Nginx, поэтому зондирующий обязан использовать реальный домен маскировки узла, а этот SNI, переданный на `dest`, вернёт только собственный сайт `dest`. Кроме того, если вы продолжите после предупреждения, узлам Xray / mihomo автоматически прописывается ограничение скорости отката REALITY (`limitFallback*` / `limit-fallback-*`)

Два известных пробела: `limitFallback` действует **на соединение**, поэтому злоупотребляющий в цикле переподключений его обходит — это подстраховка, а не решение. А у reality-инбаунда sing-box такой опции нет вовсе, из-за чего связка «sing-box + прямой выделенный порт + `dest` за CDN» самая уязвимая; обнаружив её, PSM предлагает перейти на переиспользование порта 443. Настоящее решение всегда одно — **выбрать цель маскировки, которая не стоит за CDN**.
