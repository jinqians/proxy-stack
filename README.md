...
flowchart TD
    ...
    B -->|"SNI = e.example.com（自有域名）"| F["HTTPS 伪装站点<br/>127.0.0.1:8443"]
    B -->|"SNI 未匹配任何条目"| X["直接断开<br/>不建立任何上游连接"]
...
- **未知 SNI 直接断开**：分流表只认已显式挂载的域名，扫描器随手填一个 SNI 打过来会被立刻断连，不会触发任何上游连接——这道黑洞同时堵死了「服务器被当成免费中继」的路径（见下文常见问题）
...
- **Reality 伪装域名智能发现** — 配置 Reality / XHTTP 伪装 SNI 时，可通过网络空间测绘引擎（Netlas / Quake / ZoomEye / FOFA，用你自己的 API Key，免费额度即可）自动发现与本机 **同 ASN / 同机房** 的真实 TLS 1.3 站点作为伪装目标：就近、冷门、避开被教程用烂的大厂域名；同 ASN 的主机天然不会是多租户 CDN 边缘，顺带绕开了下文常见问题里的白嫖风险。候选会在本地逐个做真实握手校验（TLS 1.3 / X25519 / 证书匹配）后才采用，并可一键批量加入上面的测活候选池。**全程不做本机端口扫描**（避免触发服务商 abuse），发现由测绘引擎的数据集完成；未配置引擎时回退为手动输入
...
### 伪装目标（`dest` / SNI）可以填 Cloudflare 后面的站点吗？

不建议，PSM 在配置时也会告警。

REALITY 的伪装机制是：任何一条**认证不通过**的连接，服务端都会原样转发给伪装目标（`dest`），让探测者看到一个真实网站完整的 TLS 响应。这个转发是无条件的——既不看来源，也不看 ClientHello 里要的是什么。

问题出在 `dest` 落到多租户 CDN 边缘（Cloudflare、Fastly、Akamai 等）上的时候。这类 IP 会按 SNI 为**任意租户**提供服务，于是任何人扫到你的 443 后，只要把 ClientHello 的 SNI 填成该 CDN 上的另一个站点，就能借你的机器建起一条通往整个 CDN 的隧道——**你的服务器变成了 CDN 的免费端口转发**，流量和账单都算你的。Xray 官方文档对此有明确警告。

PSM 分三层应对：

1. **不预置伪装域名** — 三个内核的默认伪装 SNI 都是空的，不会让一大批安装共用同一个出厂默认值（何况写死的域名日后可能自己迁到 CDN 后面）。Xray 的自动发现优先找**同 ASN** 的目标，自己机房里的主机不是 CDN 边缘；sing-box 与 mihomo 没有发现能力，因此必须显式填写，非交互 CLI 会直接报错退出，而不是拿一个借来的默认值把节点建起来
2. **配置时探测** — 填好 `dest` 后，PSM 会拿一批与它无关的探针域名当 SNI 去连同一个 IP。如果对方替一个它并不拥有的域名回了有效证书，就判定为共享前端。这测的是真正要紧的性质（这个 IP 会不会替任意租户服务），而不是去追 CDN 的 IP 段或 ASN 名单。判定是**告警而非否决**——可能误伤，风险由你自己权衡
3. **两道兜底** — 挂在 443 端口复用上时，未知 SNI 在 Nginx 层就被断掉，探测者必须用节点真实的伪装域名才走得到 REALITY，而那个 SNI 转发到 `dest` 也只能拿回 `dest` 自己的站点；此外若你在告警后选择继续，Xray / mihomo 节点会自动写入 REALITY 回落限速（`limitFallback*` / `limit-fallback-*`）

有两个已知缺口值得知道：`limitFallback` 是**每连接**语义，循环重连即可绕过，它是兜底不是解法；而 sing-box 的 reality 入站根本没有对应选项，所以「sing-box + 直连独占端口 + CDN 后面的 `dest`」是防护最薄弱的组合，PSM 遇到这个组合会额外提示改用 443 端口复用。真正管用的做法始终是**换一个不在 CDN 后面的伪装目标**。
